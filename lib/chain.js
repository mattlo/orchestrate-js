...
function Chain (o, branchConfig) {
	...
	this.o = o;
	this.currentIndex = -1;
	this.internals = {}; // used to store data
};
...
Chain.prototype.next = function (o, chain, value) {
	...
	if (nextIndex < this.steps.length) {
		this.executeStepAt(nextIndex, o, chain, value);
	} else {
		// @TODO add config that can detect if a chain needs to be deferred and not be regarded as complete
		o.eventBus().emit('chain-end', chain);
	}
};
...
Chain.prototype.executeStepAt = function (stepIndex, o, chain, value) {
	...
	if (
		value === undefined && 
		this.steps[stepIndex].config !== undefined && 
		this.steps[stepIndex].config.defaultValue !== undefined
	) {
